...
import { ArrowLeft, MoreHorizontal, Play, Trash2, Edit, FileCode, Download, Copy, Activity, Clock } from "lucide-react"
...
const getDockerScript = (id: string) => ({
  ...
  description: "A complete Node.js development environment with all necessary tools and dependencies",
...
export default function DockerScriptDetailPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen">
                  ...
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <CardDescription>Docker build script configuration</CardDescription>
                      </div>
              ...
              <TabsContent value="details">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Configuration and build information</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-6">
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      ...
                      <div>
                        ...
                        <div className="mt-1">{getBuildStatusBadge(script.buildStatus)}</div>
                      </div>
                      <div>
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      <div>
                        <p className="text-sm font-medium text-muted-foreground">Last Build</p>
                        <p className="text-sm">{script.lastBuild.toLocaleString()}</p>
                      </div>
                      <div>
                        <p className="text-sm font-medium text-muted-foreground">Build Duration</p>
                      ...
                      <div>
                        <p className="text-sm font-medium text-muted-foreground">Image URL</p>
                        <code className="text-xs bg-muted px-1 py-0.5 rounded break-all">{script.pushedImageUrl}</code>
                      </div>
                    </div>
                    ...
                    {script.testResult && (
                      <div>
                        ...
                        <div className="mt-1 p-3 bg-green-50 border border-green-200 rounded-lg">
                          <p className="text-sm text-green-800">{script.testResult}</p>
                        </div>
                      </div>
                    )}
                  </CardContent>
              ...
              <TabsContent value="history">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Previous build attempts and results</CardDescription>
                  </CardHeader>
                  <CardContent>
                      ...
                      {script.buildHistory.map((build) => (
                        <div key={build.id} className="flex items-center justify-between p-3 border rounded-lg">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-sm font-medium">Build #{build.id}</p>
                              <p className="text-xs text-muted-foreground">{build.timestamp.toLocaleString()}</p>
                            </div>
                          </div>
                          <div className="text-right">
                            <p className="text-sm">
                              <code className="bg-muted px-1 py-0.5 rounded text-xs">{build.tag}</code>
                            </p>
            ...
            {isExecuting ? (
              ...
              <Card className="h-[600px] flex flex-col">
                <CardHeader>
                  ...
                  <CardDescription>Real-time build logs will appear here when script is executed</CardDescription>
                </CardHeader>
                <CardContent className="flex-1 flex items-center justify-center">
                  <div className="text-center text-muted-foreground">
                    <Play className="h-12 w-12 mx-auto mb-4 opacity-50" />
                    <p className="text-lg font-medium">Ready to Execute</p>
                    <p className="text-sm mb-4">Click "Run Script" to start the Docker build process</p>
      ...
      <DockerScriptModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} script={script} />
    </div>
  )
}
