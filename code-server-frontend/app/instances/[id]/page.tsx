...
export default function InstanceDetailPage() {
  ...
  const recentActivities: any[] = []; //getRecentActivities();

  const [instance, setInstance] = useState<CodeServerInstance | undefined>();

  const { get: fetchCodeServerInstance } = useFetch({
    url: `${process.env.NEXT_PUBLIC_BASE_API_POINT}${API_END_POINTS.CODE_SERVER.read}`,
  });

  const loadCodeInstances = async () => {
    const response = await fetchCodeServerInstance(
      `${process.env.NEXT_PUBLIC_BASE_API_POINT}${API_END_POINTS.CODE_SERVER.read}/${instanceId}`
    );
                  ...
                  <Card>
                    ...
                    <CardContent>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Uptime: {new Date(instance.updatedAt).toLocaleString()}
                      </p>
                    </CardContent>
                  </Card>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-center py-12 text-muted-foreground">
                      ...
                      <p className="text-sm">
                        CPU, Memory, and Network usage charts will be displayed
                        here
                      </p>
                    </div>
                  </CardContent>
